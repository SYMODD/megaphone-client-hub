...
-- Migration pour corriger la fonction upsert_security_setting
...
-- Créer la fonction avec une syntaxe corrigée
...
BEGIN
  ...
  -- Calculer le hash de la nouvelle valeur avec une approche robuste
  BEGIN
    v_new_value_hash := encode(digest(p_setting_value::bytea, 'sha256'), 'hex');
  EXCEPTION WHEN OTHERS THEN
    -- Fallback sur md5 si digest échoue
