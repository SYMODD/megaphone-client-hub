...
-- Créer la fonction corrigée avec la bonne syntaxe pour digest
...
BEGIN
  ...
  -- Calculer le hash de la nouvelle valeur (syntaxe corrigée)
  v_new_value_hash := encode(digest(p_setting_value::text, 'sha256'::text), 'hex');
  ...
  IF FOUND THEN
    -- Mise à jour
    v_action := 'UPDATE';
    v_old_value_hash := encode(digest(v_old_record.setting_value::text, 'sha256'::text), 'hex');
