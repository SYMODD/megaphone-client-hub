...
-- Recréer la fonction avec une syntaxe corrigée pour digest
...
BEGIN
  ...
  -- Calculer le hash de la nouvelle valeur avec la syntaxe correcte
  v_new_value_hash := encode(digest(p_setting_value, 'sha256'), 'hex');
  ...
  IF FOUND THEN
    -- Mise à jour
    v_action := 'UPDATE';
    v_old_value_hash := encode(digest(v_old_record.setting_value, 'sha256'), 'hex');
